...
\begin{abstract}
This paper presents an analysis of the parameters of the single order Coudé CCD 700~mm spectrograph connected to the Perek 2-meter optical telescope at the Ondřejov Observatory.
...
\begin{tabular}{|c||c|c|}
\hline
	 Spectral order & $\lambda_\mathrm{min}$ ($\mathrm{\AA}$) & $\lambda_\mathrm{max}$ ($\mathrm{\AA}$) \\
\hline
\hline
	1 & 5100 & 9100 \\
\hline
...
In order to express dark current in physical units ($\mathrm{e}^{-}\mathrm{s}^{-1}$), a gain value of $0.6114~\mathrm{e}^{-}\mathrm{ADU}^{-1}$, estimated in subsequent sections, was used.
...
\begin{equation}
	...
	\label{eq:gain_eq}
\end{equation}
Here, $\overline{x}$ represents the mean signal value in ADU, $\sigma$ is its standard deviation in ADU, and $g$ is the gain in $\mathrm{e}^-\mathrm{ADU}^{-1}$.
...
Still if the gain values calculated using this method was constant for different bin sizes, it was thought it might provide some useful data.
...
\begin{enumerate}[label=\Roman*.]
    ...
    \item Each group is then processed separately:
        \begin{enumerate}[label=\arabic*.]
            ...
            \item The residual for each pixel is calculated.
...
\begin{figure}[htbp]
	...
	\end{center}
\caption{Flat-field Levels vs Exposure Time}
...
Using a simpler equation $\sigma^2 = \overline{x}/g + \sigma^2_\mathrm{ro}$ combined with the low number of data points in each bin, a perfectly straight line cannot be expected.
...
Automating this task proved to be complex, as it required determining both the linear dispersion ($\mathrm{\AA}\mathrm{px}^{-1}$) and the central wavelength ($\mathrm{\AA}$) for each frame.
If a nearly linear relationship between the grating angle and the central wavelength is assumed, then three variables must be determined for the dataset: the slope ($\mathrm{\AA}\mathrm{rad}^{-1}$), the intercept ($\mathrm{\AA}$), and the linear dispersion ($\mathrm{\AA}\mathrm{px}^{-1}$).
...
\hline
Grating Increment & Grating Angle (deg) & Min ($\mathrm{\AA}$) & Max ($\mathrm{\AA}$) \\
\hline
\hline
...
The author would like to thank the Astronomical Institute of the Czech Academy of Sciences for providing access to the Ondřejov Observatory facilities and for their support during the data processing.
