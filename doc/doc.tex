...
\IEEEauthorblockA{\textit{Mentor of the project} \\
...
\begin{abstract}
Following paper concerns itself with analysis of parameters of a coudé CCD 700~mm spectrograph connected to a Perek 2 meter optical telescope in Ondřejov Observatory. Analysis of zeros, darks and flat-fields was done in order to estimate noise parameters, pixels uniformity and sensors gain.
\end{abstract}
...
This carries out a process that enables the determination of stellar temperatures, chemical composition, radial velocities and other fundamental parameters by dispersing incoming light into its constituent wavelengths.
Nowadays, modern spectroscopic observations rely on large-aperture telescopes which are combined with sensitive CCD detectors and stable spectrographs.

As emphasized by Martinez et al. in A Practical Guide to CCD Astronomy, the scientific quality of CCD-based observations depends not only on the optical system but also on a precise understanding of detector behavior and noise sources, as well as on proper calibration procedures \cite{Martinez2001}.
Similarly, Lessons from the Masters highlights that accurate spectroscopic measurements require careful instrument characterization and calibration before any scientific interpretation can be performed \cite{Moore2014}.

In it's beginning this protocol describes the instrumental setup and theoretical background relevant to spectroscopic observations carried out with the Ondřejov 2-m telescope.
...
From the telescope three optical fibres bring the light to two different spectrographs -- an echelle spectrograph and a single order spectrograph with focal length of 700~mm (referred to as Coudé spectrograph), which is the focus of this paper.

The telescope is equipped with accurate guiding and tracking systems.
This is essential for in order o prevent blurring on long exposures when narrow entrance slits or optical fibers.
...
Grating is a prism with hundreds of grooves that acts in a similar matter to a two slit experiment -- it exploits constructive interference creating a maxima and minima for each wavelength.
Different wavelength get diffracted with different angles, for which following equation must be satisfied
\begin{equation}
	d \sin \theta_m = m \lambda,
\end{equation}
where $d$ is the distance of grooves, $\theta_m$ is a diffraction angle at which maxima occurs, $\lambda$ is lights wavelength and $m$ is a integer denoting spectral order.
Spectral orders can result in multiple wavelength interfering with each other (see \ref{img:spectral_orders}).
For example first spectral order of light with wavelength $\lambda$, will fall to the same spot as second order of wavelength $\lambda/2$ \cite{tau_hydrogen_2018}.
...
In order to prevent interference between spectral orders multiple different constructions can be used.
Echelle type spectrographs use cross-dispersing element to spatially separate diffraction orders on the detector -- this enables them to measure very wide spectrum in a single image \cite{Eversberg2015}.
A simpler approach is to use filters to block out light of wavelength that would interfere with the observation.
It's also possible to do observation in an order where possible interfering wavelengths fall bellow capabilities of the CCD detector or the optical system itself.
Coudé spectrograph connected to Perek telescope employs both approaches \cite{Ondrejov2mCZ}.

\section{Coudé Spectrograph}

The coudé spectrograph at the Ondřejov Observatory operates as a single order spectrograph operating either in the first or the second order depending on what range of wavelengths is being observed \ref{tab:spec_orders}.
...
Beam then encounters the diffraction grating is encountered, which separates the light according to wavelength.
Next the dispersed light is focused onto the CCD detector by Schmidt camera optics, where the spectrum is recorded.
Schmidt camera is used as to prevent distortion of the spectrum towards edges of the sensor.
...
And as opposed to CMOS sensor they offer more consistency between pixels as pixels share a read out circuitry.
Thus CCD senors often exhibit lesser fixed pattern than CMOS detectors.

The basic operating principle is that incident photons interact with the silicon substrate, generating photoelectrons through the photoelectric effect.
...
Then, using a sequence of clocked voltage shifts, the stored charge is transferred across the detector, amplified and read out using ADC \cite{Moore2014}.
...
\label{eq:gain_def}
\end{equation}
where $g$ is the gain in $\mathrm{e}^{-}\mathrm{ADU}^{-1}$ and $S_{\text{ADU}}$ recorded signal in ADU.
Knowledge of sensor's gain is essential for converting measured signals into physically meaningful quantities and for correctly estimating the contributions of different noise sources.
...
Under ideal conditions gain would be a constant and thus CCD sensor would behave linearly.
However often gain is a function of the incoming light intensity or other parameters.
Additionally commonly even when gain isn't dependant on light intensity, deviation from linearity may still occur at very low signal levels due to electronic effects or near full-well capacity due to charge saturation.
...
The main contributions are read noise, dark current noise, and photon (shot) noise.
...
Lastly, the other noise source that can be found is the photon noise, sometimes referred to just as shot noise, which originates from the statistical nature of photon arrival at the detector -- thus it isn't a property of the detector itself.
...
Photon noise becomes the dominant noise source at high fluxes since it increases with signal level.
Compared to read noise and dark current noise, photon noise represents a fundamental physical limit and cannot be reduced by just making instrumental improvements.
The way to reduce its relative impact would be by increasing the signal level where it is stated that it can be reduced through longer exposure times or larger telescope apertures.

Finally, these different noise sources must be considered together.
Assuming the noise sources are statistically independent, the total noise in a CCD pixel is given by the quadratic sum of all contributions:
\begin{equation}
...
Different types of calibration frames are essential to estimate sensors properties and separate the true astronomical signal from the detector and instrumental related contributions.
The most used ones are bias (zero) frames, dark frames and flat-field frames.
...
As stated in, by combining multiple bias frames, a master bias frame can be constructed.
This frame is  subsequently subtracted from all other images to remove any sensors fixed pattern \cite{Moore2014}.
...
Dark frames can also capture fixed-pattern components of the dark signal, such as hot pixels. These can be removed by subtracting a master dark frame from the data \cite{Martinez2001}.

Flat-field frames are used to correct for pixel-to-pixel sensitivity variations, large-scale illumination gradients across the detector and estimate sensors gain.
Usually a uniform illumination of the sensor is required to ascertain these parameters.
However due to inherent nature of spectrographs, light is decomposed to its constituent wavelength, such uniform illumination is impossible to achieve \cite{Robertson_2021}.
This imposes large complications on flat-field analysis.
...
In non where sensor installation can enable a uniforma flat-fields illumination good gain estimate can be made from just a couple of images\cite{irafGain}.
...
Due to the failure of the model based on most numerous exposure time an model based on mean of all flat-fields was used.
...
Another step in processing is scaling the model to each flat-field level.
This isn't particularly complicated as flat-fields energy raises rather linearly with exposure time as seem on picture \ref{img:exposure_time}.
...
In the model itself these inilluminated parts are also zeroed so that after scaling them the values don't rise, same way they don't rise in the real flat-fields for longer exposures.
...
Intra-frame methods couldn't work due to nature of spectostrophic images where flat-fields illumination is unequal.
