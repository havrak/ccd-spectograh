...
\begin{tabular}{|c||c|c|}
\hline
	 Spectral order & $\lambda_\mathrm{min}$ (A) & $\lambda_\mathrm{max}$ (A) \\
\hline
	1 & 5100 & 9100 \\
\hline
	2 & 3700 & 5100 \\
...
% \begin{figure}[htbp]
% \centering
% \includegraphics[width=0.95\columnwidth]{../img/im10.jpg}
...
% \begin{figure}[htbp]
% \centering
% \includegraphics[width=0.95\columnwidth]{../img/im11.jpg}
...
In order to express dark currents in accurate units $e^{-}s^{-1}$ gain value of $0.555\mathrm{e}^{-}\mathrm{ADU}^{-1}$, estimated in following chapters, was used.
...
It's apparent that the model work really well for low values -- where the points follow the square root dependance rather well, however for larger values model breaks down.
Models failure can also be demonstrated while on residual histograms (images \ref{img:flat_hist_10s_med} and \ref{img:flat_hist_10s_3rd}), where not only shape of the curve itself is wrong but also mean estimate isn't clearly correct.
