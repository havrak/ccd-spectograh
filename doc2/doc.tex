...
\begin{equation}
	...
	\label{eq:gain_eq}
\end{equation}
As noise is statistically independent it's simply included in the sum, $\overline{x}$ is mean signal value, $\sigma$ it's standard deviation and lastly $g$ is gain\cite{Newberry1996}.
...
Incoming light thus still comes through the spectral dispersive grating and different parts of spectograph are then illuminated with different light intensities and at different wavelength.
As light source doesn't have an constant spectrum illumination will thus be strictly uniform.
